...
class Artifact:
    ...
    def __repr__(self):
        return f'<{type(self).__name__} obj:{self.obj} tags:{self.tags}'
...
class SimpleRule(Rule):
    def __init__(self, source_pattern, target_pattern, function, del_ext=False):
    ...
    def apply(self, artifacts):
        for artifact in (x for x in artifacts if self.source_patterns[0].match(x)):
            artifact.data = self.function(artifact.data)
            if self.del_ext:
